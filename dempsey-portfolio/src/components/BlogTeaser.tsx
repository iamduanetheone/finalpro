          ...
          {posts.length > 0 ? (
            posts.map((post) => (
              ...
              >
                ...
                <p className="text-slate-600 mb-4 flex-grow text-sm leading-relaxed">{post.excerpt || 'Read this article for expert SEO copywriting insights...'}</p>
                <Link
                  href={`/blog/${post.categories && post.categories[0] ? `${post.categories[0].title.toLowerCase()}/` : ''}${post.slug.current}`}
            ...
            <div className="col-span-3 text-center py-8">
              <p className="text-slate-600">Blog posts coming soon! Check back later for insightful content.</p>
            </div>
          )}
        </div>
